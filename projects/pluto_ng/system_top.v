...
module system_top (
  ...
  reg         mcs_out = 1'b0;
  reg [31:0]  system_sync_debounce_cnt = 32'd0;
  reg         system_sync = 1'd0;
  ...
  always @(posedge fpga_ref_clk) begin
    ...
    if (mcs_start & mcs_or_system_sync) begin
      mcs_sync_busy <= 1'b1;
    ...
    if (sys_start & !mcs_or_system_sync) begin
      system_sync_debounce_cnt <= 32'h4000000; // aprox 1,7s
      system_sync <= 1'b1;
    end else if (system_sync_debounce_cnt != 32'd0) begin
      system_sync_debounce_cnt <= system_sync_debounce_cnt + 1'b1;
      system_sync <= 1'b0;
    end
  end

  assign mcs_start = !mcs_sync_m & fpga_mcs_in & !mcs_sync_busy;
  assign sys_start = !mcs_sync_m & fpga_mcs_in & system_sync_debounce_cnt != 'd0;
  assign mcs_or_system_sync = gpio_o[64];
