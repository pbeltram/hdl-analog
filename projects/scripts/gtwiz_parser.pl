...
sub get_attribute_value {
  ...
  foreach my $param (@$param_list) {
    if ($xcvr_params_str =~ m/($xcvr_type\_$param.*\n)/i) {
      my $value = $1;
      $value =~ s/${xcvr_type}_//;
...
sub gen_drp_cmd {
  ...
  for my $i (0 .. $#File) {
    ...
    $param_value =~ s/\).*$//;
    chomp($param_value);
    ## ignore all attributes which are related to unused features and double
    ## check attribute validity
    if ($param_name =~ /^(?!ES_)(?!PCIE_)(?!TXPH_)(?!TXPI_)(?!TX_PI_BIASSET)(?!PREIQ_FREQ)(?!CH_HSPMUX)/) {
...
sub parse_pll_dividers {
  ...
  foreach my $pll_dividers_key (keys %pll_dividers) {
    if (exists $$cm_params_ref{$pll_dividers_key}) {
      $pll_dividers{$pll_dividers_key} = $$cm_params_ref{$pll_dividers_key};
    }
    if (exists $$ch_params_ref{$pll_dividers_key}) {
      $pll_dividers{$pll_dividers_key} = $$ch_params_ref{$pll_dividers_key};
    }
  }
  ...
  foreach my $pll_dividers_key (keys %pll_dividers) {
    print WRITEFILE "$pll_dividers_key = $pll_dividers{$pll_dividers_key}\n";
  }
...
sub parse_gt {
  ...
  my $xcvr_common  = parse_gt_attribute($xcvr_type, "common", $hdl_path);
  my $xcvr_channel = parse_gt_attribute($xcvr_type, "channel", $hdl_path);
  ...
  for my $i (0 .. $#gt_attributes) {
    ...
    for my $j (0 .. $#gt_configs) {
      ...
      foreach my $type_key (keys %{ $$gt_drp_ref{$gt_configs[$j]}}) {
        if (exists $$gt_drp_ref{$gt_configs[$j]}{$type_key}{$gt_attributes[$i]}) {
          if (oct(lc($$gt_drp_ref{$gt_configs[$j]}{$type_key}{$gt_attributes[$i]})) != oct($default_value)) {
            print MYTABLE "$$gt_drp_ref{$gt_configs[$j]}{$type_key}{$gt_attributes[$i]},\t";
          } else {
            print MYTABLE ",\t";
          }
          $is_exist = 1;
      ...
      foreach my $gt_drp_config_key (keys %{ $gt_drp_ref }) {
        if ($gt_drp_config_key ne $ref_conf) {
          if (exists $$gt_drp_ref{$gt_drp_config_key}{$gt_ref_conf_type_key}{$gt_ref_conf_attribute_key}) {
            ...
            if ($$gt_drp_ref{$ref_conf}{$gt_ref_conf_type_key}{$gt_ref_conf_attribute_key} ne
...
##	print "gt_global value is: $$gt_drp_ref{$ref_conf}{$gt_ref_conf_type_key}{$gt_ref_conf_attribute_key} attributes: $ref_conf , $gt_ref_conf_type_key, $gt_ref_conf_attribute_key\n";
...
##	  print "deleted value is: $$gt_drp_ref{$gt_drp_config_key}{$gt_ref_conf_type_key}{$gt_ref_conf_attribute_key} attributes: $gt_drp_config_key, $gt_ref_conf_type_key, $gt_ref_conf_attribute_key\n";
...
## Currently only GTHE3 and GTHE4 is supported
print "NOTE: To parse the generate GT IP change directory to *.srcs/sources_1/ip/ ...\n";
if (not defined $xcvr_type) {
  die "ERRO Need to define an XCVR type. (GTHE3 or GTHE4)";
} elsif ((uc($xcvr_type) ne "GTHE3") and (uc($xcvr_type) ne "GTHE4") and (uc($xcvr_type) ne "GTYE3") and (uc($xcvr_type) ne "GTYE4")) {
  die "ERROR Wrong XCVR type, should be GTHE3 or GTHE4";
}
